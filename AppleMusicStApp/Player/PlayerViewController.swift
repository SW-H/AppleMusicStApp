...
class PlayerViewController: UIViewController { // 곡 정보 표시, 곡 재생
    ...
    override func viewDidLoad() {
        ...
        // CMTime : Core Media Time - AVFoundation 더 밑에 있는 것
        // 0.1초 = 기준시간(seconds)를 몇개로(preferredTimescale) 분할 할 것이냐
        timeObserver = simplePlayer.addPeriodicTimeObserver(forInterval: CMTime(seconds: 1, preferredTimescale: 10),
                                                            queue: DispatchQueue.main ,
                                                            using: { time in self.updateTime(time: time) }//현재시간을 받아서 업데이트
                                                            )
    ...
    @IBAction func seek(_ sender: UISlider) {
        ...
        let seconds = position * currentItem.duration.seconds // 재생시키고 싶은 값, 90.123143221 이런식으로 나옴
        let time = CMTime(seconds: seconds, preferredTimescale: 100)// seconds -> CMTime, 소수점아래 두자리까지만 쓰겠다
...
extension PlayerViewController {
    ...
    func updateTime(time: CMTime) {
        ...
        if isSeeking == false {// 노래 들으면서 시킹하면, 자꾸 슬라이더가 업데이트 됨, 따라서 시킹아닐때만 슬라이더 업데이트하자
            // TODO: 슬라이더 정보 업데이트
